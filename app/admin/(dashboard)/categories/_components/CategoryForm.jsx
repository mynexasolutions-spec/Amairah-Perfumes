...
export default function CategoryForm({ category }) {
  ...
  return (
    <form
      action={formAction}
      className="max-w-3xl space-y-6 rounded-[2.5rem] border border-gold-400/10 bg-gradient-to-b from-[#120f0d]/90 via-[#0b0a0a]/90 to-[#080707]/95 p-6 md:p-8 backdrop-blur-xl shadow-2xl"
    >
        ...
        <div className="space-y-5">
          <div>
            ...
            {name && (
              <p className="mt-2 text-sm text-ivory/30">
                URL Preview: <span className="font-mono text-gold-300/60">/shop?category={slugPreview(name)}</span>
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
            <div>
              ...
              <p className="mt-2 text-sm text-ivory/30">Lower numbers appear first on the store.</p>
            </div>
      ...
      <div className="flex items-center gap-4 border-t border-gold-400/10 pt-6">
        ...
        <Link 
          ...
          className="text-xs uppercase tracking-widest font-semibold text-ivory/40 hover:text-gold-300 transition-colors duration-300"
        >
          Cancel
        </Link>
      </div>
