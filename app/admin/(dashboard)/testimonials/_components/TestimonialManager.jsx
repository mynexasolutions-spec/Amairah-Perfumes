        ...
        <button
          ...
          className={`rounded-xl border px-4 py-2.5 text-sm font-medium transition-colors duration-300 ${
            Number(value) === r
              ...
              : "border-gold-400/10 bg-ink/40 text-ivory/60 hover:border-gold-400/20"
          }`}
        >
          {r}★
        </button>
...
function TestimonialEditForm({ testimonial, onCancel, onSaved }) {
  ...
  return (
    <form onSubmit={handleSave} className="w-full space-y-3 rounded-2xl border border-gold-400/20 bg-white/[0.03] p-4">
      ...
      <div>
        <label className="mb-1.5 block text-sm font-semibold uppercase tracking-wide text-ivory/40">Rating</label>
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,360px)]">
      <div className={`${panelClass} order-2 min-w-0 p-6 md:p-8 lg:order-1`}>
        <h2 className="mb-4 font-display text-lg text-ivory">Existing Testimonials</h2>
        {testimonials.length === 0 ? (
          <p className="py-8 text-center text-sm text-ivory/40">No testimonials yet — the homepage will show default sample quotes.</p>
        ) : (
              ...
              editingId === t.id ? (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-sm text-ivory">{t.customer_name} {t.location && <span className="text-ivory/40">· {t.location}</span>}</p>
                    <p className="truncate text-sm text-ivory/40">{t.review_text}</p>
                  </div>
      ...
      <form onSubmit={handleAdd} className={`${panelClass} order-1 h-fit min-w-0 space-y-4 p-6 lg:order-2`}>
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-display text-base text-ivory">Add Testimonial</h2>
        </div>
        <ImageUploader value={imageUrl} onChange={setImageUrl} folder="amairah/testimonials" />
        <input placeholder="Customer Name" value={form.customer_name} onChange={update("customer_name")} className={inputClass} required />
        <input placeholder="Location (e.g. Delhi)" value={form.location} onChange={update("location")} className={inputClass} />
