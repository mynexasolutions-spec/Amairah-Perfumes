...
export default function AnnouncementForm({ announcements }) {
  ...
  return (
    <div className="space-y-6">
      <form
        onSubmit={handleAdd}
        className="flex flex-col gap-3 rounded-[2rem] border border-gold-400/10 bg-gradient-to-b from-ink-soft/80 to-ink-soft/30 p-6 backdrop-blur-md sm:flex-row sm:items-center"
      >
        ...
        <input
          ...
          className="flex-1 rounded-xl border border-gold-400/10 bg-ink/40 px-4 py-2.5 text-sm text-ivory placeholder:text-ivory/30 transition-colors duration-300 focus:border-gold-400/40 focus:outline-none focus:ring-1 focus:ring-gold-400/20 hover:border-gold-400/20"
        />
      ...
      {announcements.length === 0 ? (
        ...
        <ul className="space-y-3">
          {announcements.map((a) => (
            ...
            >
              <p className="flex-1 text-sm text-ivory">{a.message}</p>
