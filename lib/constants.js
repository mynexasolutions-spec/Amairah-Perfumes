...
// Bundle items already get their own bundle-price discount, so they're
// excluded here — otherwise a completed bundle would double-dip into the
// separate bulk/quantity discount tiers too.
export function nonBundleCartQuantity(items) {
  return (items || []).filter((i) => !i.bundleGroupId).reduce((sum, i) => sum + i.quantity, 0);
}
