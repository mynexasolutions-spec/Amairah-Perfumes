 ...
 */
export default function ImageUploader({ value, onChange, multiple = false, folder = "amairah" }) {
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-3">
        {urls.map((url, idx) => (
          <div key={url} className="relative h-24 w-24 overflow-hidden rounded-xl border border-ink-line">
          ...
          >
            {({ open }) => (
              <button
                type="button"
                onClick={() => open()}
                className="flex h-24 w-24 flex-col items-center justify-center gap-1.5 rounded-xl border border-dashed border-ink-line text-ivory/40 transition-colors hover:border-gold-400/50 hover:text-gold-300"
              >
